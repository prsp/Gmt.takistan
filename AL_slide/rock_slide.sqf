...
fnc_rock_and_roll = {
	...
	_miez = _this select 0;
	_cover = _this select 1;
	_start_point = _this select 2;
	_dir_x = _this select 3;
	_dir_y = _this select 4;
	_acc = _this select 5;
	_alt_jump = _this select 6;
	_al_acc = _this select 7;
	...
	while {(abs(_unghi) > 9) and (_start_point getVariable "started")} do 
	{
		...
		[[_cover],"AL_slide\impact_dust.sqf"] remoteExec ["execVM",0];
		waitUntil {((getposatl _cover select 2)< 0.2) or !(_start_point getVariable "started")};
		_sunet_impact_bol = ["01_impact_stone","02_impact_stone","03_impact_stone","04_impact_stone","05_impact_stone","06_impact_stone"] call BIS_fnc_selectRandom;
...
_start_point	= _this select 0;
_danger			= _this select 1;
_nr_rocks 		= _this select 2;
_acc	 		= _this select 3;
_alt_jump		= _this select 4;
_part_sp		= _this select 5;
...
[[_start_point,_dir_x,_dir_y,_dir_slide,_part_sp],"AL_slide\rock_roll.sqf"] remoteExec ["execVM",0];
	...
	while {_nr_rocks>0} do 
		{
			...
			_fct_poz = (getdir _start_point)+([-1,1] call BIS_fnc_selectRandom) * (random 90);
